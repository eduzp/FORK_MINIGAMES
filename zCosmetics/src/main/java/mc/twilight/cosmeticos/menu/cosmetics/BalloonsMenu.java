...
public class BalloonsMenu extends PagedPlayerMenu {
  ...
  private Map<ItemStack, BalloonCosmetic> balloons;
  
  public BalloonsMenu(CUser user) {
    super(user.getPlayer(), "Balões", 6);
    ...
    for (BalloonCosmetic cosmetic : Cosmetic.listCosmetics(BalloonCosmetic.class)) {
      ...
      if (user.isSelected(cosmetic)) {
        ...
        loreList.add("");
        loreList.add("§cVocê não possui esse balão.");
      }
      ...
      items.add(icon);
      this.balloons.put(icon, cosmetic);
    }
    
    this.removeSlotsWith(BukkitUtils.deserializeItemStack("INK_SACK:1 : 1 : nome>&cVoltar"), 48);
    this.removeSlotsWith(BukkitUtils.deserializeItemStack("BARRIER : 1 : nome>&cRemover Balão"), 49);
    this.setItems(items);
      ...
      if (evt.getWhoClicked().equals(this.player)) {
        ...
        if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(this.getCurrentInventory())) {
          if (evt.getSlot() == nextPage) {
            ...
            if (!user.isSelected(Cosmetic.NONE_BALLOON)) {
              user.selectCosmetic(Cosmetic.NONE_BALLOON);
              user.handleBalloon();
              new BalloonsMenu(user);
            }
          } else {
            BalloonCosmetic cosmetic = balloons.get(evt.getCurrentItem());
            if (cosmetic != null) {
              if (user.hasCosmetic(cosmetic)) {
                EnumSound.NOTE_PLING.play(player, 1.0F, 2.0F);
                if (user.isSelected(cosmetic)) {
