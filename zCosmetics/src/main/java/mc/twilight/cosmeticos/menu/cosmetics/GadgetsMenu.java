...
public class GadgetsMenu extends PagedPlayerMenu {
  
  private Map<ItemStack, GadgetsCosmetic> gadgets;
  
  public GadgetsMenu(CUser user) {
    super(user.getPlayer(), "Engenhocas", 6);
    ...
    for (GadgetsCosmetic cosmetic : Cosmetic.listCosmetics(GadgetsCosmetic.class)) {
      ...
      if (user.isSelected(cosmetic)) {
        ...
        loreList.add("");
        loreList.add("§cVocê não possui essa engenhoca.");
      }
      ...
      items.add(icon);
      this.gadgets.put(icon, cosmetic);
    }
    
    this.removeSlotsWith(BukkitUtils.deserializeItemStack("INK_SACK:1 : 1 : nome>&cVoltar"), 48);
    this.removeSlotsWith(BukkitUtils.deserializeItemStack("BARRIER : 1 : nome>&cRemover Engenhoca"), 49);
    this.setItems(items);
      ...
      if (evt.getWhoClicked().equals(this.player)) {
        ...
        if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(this.getCurrentInventory())) {
          if (evt.getSlot() == nextPage) {
            ...
            if (!user.isSelected(Cosmetic.NONE_GADGET)) {
              user.selectCosmetic(Cosmetic.NONE_GADGET);
              user.handleGadget();
              new GadgetsMenu(user);
            }
          } else {
            GadgetsCosmetic cosmetic = gadgets.get(evt.getCurrentItem());
            if (cosmetic != null) {
              if (user.hasCosmetic(cosmetic)) {
                EnumSound.NOTE_PLING.play(player, 1.0F, 2.0F);
