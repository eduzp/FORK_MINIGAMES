...
public class ParticlesMenu extends PagedPlayerMenu {
  
  private Map<ItemStack, ParticleCosmetic> particles;
  
  public ParticlesMenu(CUser user) {
    super(user.getPlayer(), "Partículas", 6);
    ...
    for (ParticleCosmetic cosmetic : Cosmetic.listCosmetics(ParticleCosmetic.class)) {
      ...
      if (user.isSelected(cosmetic)) {
        ...
        loreList.add("");
        loreList.add("§cVocê não possui essa partícula.");
      }
      ...
      items.add(icon);
      this.particles.put(icon, cosmetic);
    }
    
    this.removeSlotsWith(BukkitUtils.deserializeItemStack("INK_SACK:1 : 1 : nome>&cVoltar"), 48);
    this.removeSlotsWith(BukkitUtils.deserializeItemStack("BARRIER : 1 : nome>&cRemover Partícula"), 49);
    this.setItems(items);
      ...
      if (evt.getWhoClicked().equals(this.player)) {
        ...
        if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(this.getCurrentInventory())) {
          if (evt.getSlot() == nextPage) {
            ...
            if (!user.isSelected(Cosmetic.NONE_PARTICLE)) {
              user.selectCosmetic(Cosmetic.NONE_PARTICLE);
              user.handleParticle();
              new ParticlesMenu(user);
            }
          } else {
            ParticleCosmetic cosmetic = particles.get(evt.getCurrentItem());
            if (cosmetic != null) {
              if (user.hasCosmetic(cosmetic)) {
                EnumSound.NOTE_PLING.play(player, 1.0F, 2.0F);
                if (user.isSelected(cosmetic)) {
