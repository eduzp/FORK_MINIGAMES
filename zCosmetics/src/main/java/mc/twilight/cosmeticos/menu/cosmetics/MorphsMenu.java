...
public class MorphsMenu extends PagedPlayerMenu {
  
  private Map<ItemStack, MorphCosmetic> hats;
  
  public MorphsMenu(CUser user) {
    super(user.getPlayer(), "Mutações", 6);
    ...
    for (MorphCosmetic cosmetic : Cosmetic.listCosmetics(MorphCosmetic.class)) {
      ...
      if (user.isSelected(cosmetic)) {
        ...
        loreList.add("");
        loreList.add("§cVocê não possui essa mutação.");
      }
    ...
    this.removeSlotsWith(BukkitUtils.deserializeItemStack("BARRIER : 1 : nome>&cRemover Mutação"), 49);
    this.setItems(items);
      ...
      if (evt.getWhoClicked().equals(this.player)) {
        ...
        if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(this.getCurrentInventory())) {
          if (evt.getSlot() == nextPage) {
            ...
            if (!user.isSelected(Cosmetic.NONE_MORPH)) {
              user.selectCosmetic(Cosmetic.NONE_MORPH);
              user.handleMorph();
              new MorphsMenu(user);
            }
          } else {
            MorphCosmetic cosmetic = hats.get(evt.getCurrentItem());
            if (cosmetic != null) {
              if (user.hasCosmetic(cosmetic)) {
                if (user.isSelected(cosmetic)) {
                  EnumSound.NOTE_PLING.play(player, 1.0F, 2.0F);
