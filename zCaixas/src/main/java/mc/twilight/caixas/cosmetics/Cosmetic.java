...
import mc.twilight.caixas.hook.container.CosmeticsContainer;
...
public abstract class Cosmetic {
  ...
  protected static final List<Cosmetic> COSMETICS = new ArrayList<>();
  protected long cash;
  ...
  public static Cosmetic findById(String lootChestID) {
    return COSMETICS.stream().filter(cosmetic -> cosmetic.getMysteryBoxID().equals(lootChestID)).findFirst().orElse(null);
  }
  ...
  public void give(Profile profile) {
    profile.getAbstractContainer("zCaixas", "cosmetics", CosmeticsContainer.class).addCosmetic(this);
  }
  
  public boolean has(Profile profile) {
    return profile.getAbstractContainer("zCaixas", "cosmetics", CosmeticsContainer.class).hasCosmetic(this);
  }
  
  public boolean isSelected(Profile profile) {
    return profile.getAbstractContainer("zCaixas", "selected", SelectedContainer.class).isSelected(this);
  }
  ...
  public String getMysteryBoxID() {
    return "mb" + this.type.ordinal() + "-" + this.id;
  }
