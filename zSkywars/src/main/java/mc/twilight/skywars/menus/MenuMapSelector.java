...
public class MenuMapSelector extends UpdatablePlayerPagedMenu {
              ...
              if (mapName != null && this.can) {
                ...
                for (AbstractSkyWars game : this.games.get(mapName)) {
                  if (game.getState().canJoin() && game.getOnline() < game.getMaxPlayers()) {
                    ...
                    this.profile.setStats("zCoreSkyWars", System.currentTimeMillis() + TimeUtils.getExpireIn(1), "lastmap");
  ...
  @Override
  public void update() {
    if (!this.player.hasPermission("zskywars.menu.selector") && profile.getStats("zCoreSkyWars", "lastmap") >= System.currentTimeMillis()) {
      this.can = false;
    }
    ...
    this.removeSlotsWith(BukkitUtils.deserializeItemStack(Language.lobby$npc$play$menu$info$item.replace("{desc}", this.player.hasPermission("zskywars.menu.selector") ?
