...
import mc.twilight.skywars.game.events.AnnounceEvent;
import mc.twilight.skywars.game.interfaces.LoadCallback;
import mc.twilight.skywars.game.object.SkyWarsBlock;
import mc.twilight.skywars.game.object.SkyWarsChest;
import mc.twilight.skywars.game.object.SkyWarsConfig;
import mc.twilight.skywars.game.object.SkyWarsTask;
...
import static mc.twilight.skywars.hook.SWCoreHook.reloadScoreboard;
...
public abstract class AbstractSkyWars implements Game<SkyWarsTeam> {
  ...
  public static void setupGames() {
    ...
    File ymlFolder = new File("plugins/zSkyWars/arenas");
    File mapFolder = new File("plugins/zSkyWars/mundos");
  ...
  public static void load(File yamlFile, LoadCallback callback) {
    ...
    try {
      File backup = new File("plugins/zSkyWars/mundos", arenaName);
  ...
  private void joinParty(Profile profile, boolean ignoreLeader) {
    ...
    if (team.listPlayers().size() == 1) {
      team.buildCage(profile.getAbstractContainer("zCoreSkyWars", "selected", SelectedContainer.class).getSelected(CosmeticType.CAGE, Cage.class));
    }
      ...
      if (alive && this.state == GameState.EMJOGO) {
        ...
        for (Profile hitter : hitters) {
          if (!hitter.equals(killer) && hitter.playingGame() && hitter.getGame().equals(this) && !this.isSpectator(hitter.getPlayer())) {
            hitter.addStats("zCoreSkyWars", this.getMode().getStats() + "assists");
            // Mensal.
            hitter.addStats("zCoreSkyWars", "monthlyassists");
          }
        }
        hitters.clear();
      }
  ...
  public void start() {
    ...
    for (Player player : this.listPlayers(false)) {
      ...
      profile.addStats("zCoreSkyWars", this.getMode().getStats() + "games");
      if (Main.kClans) {
      ...
      Kit kit = profile.getAbstractContainer("zCoreSkyWars", "selected", SelectedContainer.class).getSelected(CosmeticType.KIT, Kit.class, this.getMode().getCosmeticIndex());
      if (kit != null) {
        kit.apply(profile);
      }
  ...
  public void killLeave(Profile profile, Profile killer) {
    ...
    profile.addStats("zCoreSkyWars", this.getMode().getStats() + "deaths");
    profile.addStats("zCoreSkyWars", "monthlydeaths");
    if (pk == null) {
      ...
      if (player.getLastDamageCause() == null || player.getLastDamageCause().getCause() != EntityDamageEvent.DamageCause.VOID) {
        KillEffect ke = killer.getAbstractContainer("zCoreSkyWars", "selected", SelectedContainer.class).getSelected(CosmeticType.KILL_EFFECT, KillEffect.class);
        if (ke != null) {
      ...
      if (this.getMode() == SkyWarsMode.RANKED) {
        killer.addStats("zCoreSkyWars", Language.options$points$kills, "rankedpoints");
      }
      killer.addCoinsWM("zCoreSkyWars", Language.options$coins$kills);
      killer.addStats("zCoreSkyWars", this.getMode().getStats() + "kills");
      killer.addStats("zCoreSkyWars", "monthlykills");
      if (Main.kClans) {
      ...
      DeathMessage dm = killer.getAbstractContainer("zCoreSkyWars", "selected", SelectedContainer.class).getSelected(CosmeticType.DEATH_MESSAGE, DeathMessage.class);
      if (dm != null) {
    ...
    DeathCry dc = profile.getAbstractContainer("zCoreSkyWars", "selected", SelectedContainer.class).getSelected(CosmeticType.DEATH_CRY, DeathCry.class);
    if (dc != null) {
      dc.getSound().play(player.getWorld(), player.getLocation(), dc.getVolume(), dc.getSpeed());
    }
    DeathHologram dh = profile.getAbstractContainer("zCoreSkyWars", "selected", SelectedContainer.class).getSelected(CosmeticType.DEATH_HOLOGRAM, DeathHologram.class);
    if (dh != null) {
  ...
  public void stop(SkyWarsTeam winners) {
    ...
    for (Player player : this.listPlayers(false)) {
      ...
      if (winners != null && winners.hasMember(player)) {
        profile.addCoinsWM("zCoreSkyWars", Language.options$coins$wins);
        if (this.getMode().equals(SkyWarsMode.RANKED)) {
          profile.addStats("zCoreSkyWars", Language.options$points$wins, "rankedpoints");
        }
        if (Main.kClans) {
          if (ClanAPI.getClanByPlayerName(profile.getName()) != null) {
        ...
        profile.addStats("zCoreSkyWars", this.getMode().getStats() + "wins");
        profile.addStats("zCoreSkyWars", "monthlywins");
