package mc.twilight.skywars.hook;
...
import mc.twilight.skywars.hook.hotbar.SWHotbarActionType;
import mc.twilight.skywars.hook.protocollib.HologramAdapter;
      ...
      public void update() {
        for (int index = 0; index < Math.min(lines.size(), 15); index++) {
          ...
          if (game != null) {
            ...
            line = line.replace("%zCore_SkyWars_league%", SkyWarsLeague.getLeague(profile).getTag());
          }
