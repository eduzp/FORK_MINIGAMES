...
public class LeaderboardCommand extends SubCommand {
  ...
  public void perform(Player player, String[] args) {
    if (args.length == 0) {
      player.sendMessage(" \n§eAjuda\n \n§6/bw leaderboard adicionar [id] [tipo] §f- §7Adicionar Leaderboard.\n§6/bw leaderboard remover [id] §f- §7Remover Leaderboard.\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("adicionar")) {
      if (args.length <= 2) {
        player.sendMessage("§cUtilize /bw leaderboard adicionar [id] [vitorias/camas/abates]");
        return;
      }
      ...
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /bw leaderboard remover [id]");
        return;
      }
