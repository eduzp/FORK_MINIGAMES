...
import mc.twilight.bedwars.lobby.StatsNPC;
...
public class NPCStatsCommand extends SubCommand {
  ...
  public void perform(Player player, String[] args) {
    if (args.length == 0) {
      player.sendMessage(" \n§eAjuda\n \n§6/bw npcstatus adicionar [id] §f- §7Adicionar NPC.\n§6/bw npcstatus remover [id] §f- §7Remover NPC.\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("adicionar")) {
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /bw npcstatus adicionar [id]");
        return;
      }
      ...
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /bw npcstatus remover [id]");
        return;
      }
