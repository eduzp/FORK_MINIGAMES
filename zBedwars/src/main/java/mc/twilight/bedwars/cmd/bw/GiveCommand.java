...
import mc.twilight.core.booster.Booster.BoosterType;
...
public class GiveCommand extends SubCommand {
  ...
  public void perform(CommandSender sender, String[] args) {
    if (args.length <= 1) {
      sender.sendMessage(" \n§eAjuda - Dar\n \n§6/bw dar [jogador] coins [quantia]\n§6/bw dar [jogador] booster [private/network] [multiplicador] [horas]\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("booster")) {
      if (args.length < 5) {
        sender.sendMessage("§cUtilize /bw dar [jogador] booster [private/network] [multiplicador] [horas]");
        return;
      }
      ...
      if (args.length < 3) {
        sender.sendMessage("§cUtilize /bw dar [jogador] coins [quantia]");
        return;
      }
      ...
      try {
        ...
        target.addCoins("zCoreBedWars", coins);
