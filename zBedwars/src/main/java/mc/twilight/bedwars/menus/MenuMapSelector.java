...
public class MenuMapSelector extends UpdatablePlayerPagedMenu {
  ...
  private Map<String, List<BedWars>> games = new HashMap<>();
  
  public MenuMapSelector(Profile profile, BedWarsMode mode) {
    super(profile.getPlayer(), "Mapas - BedWars " + mode.getName(), 6);
              ...
              if (mapName != null && this.can) {
                ...
                for (BedWars game : this.games.get(mapName)) {
                  if (game.getState().canJoin() && game.getOnline() < game.getMaxPlayers()) {
                    ...
                    this.profile.setStats("zCoreBedWars", System.currentTimeMillis() + TimeUtils.getExpireIn(1), "lastmap");
  ...
  @Override
  public void update() {
    if (!this.player.hasPermission("zbedwars.menu.selector") && profile.getStats("zCoreBedWars", "lastmap") >= System.currentTimeMillis()) {
      this.can = false;
    }
    
    List<ItemStack> items = new ArrayList<>();
    this.games = BedWars.getAsMap(this.mode);
    for (Map.Entry<String, List<BedWars>> entry : this.games.entrySet()) {
      List<BedWars> games = entry.getValue();
    ...
    this.removeSlotsWith(BukkitUtils.deserializeItemStack(Language.lobby$npc$play$menu$info$item.replace("{desc}", this.player.hasPermission("zbedwars.menu.selector") ?
