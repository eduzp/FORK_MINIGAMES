package mc.twilight.bedwars.hook.mb;
  ...
  public static void sync(CommandSender sender) {
    if (Database.getInstance() instanceof MongoDBDatabase) {
      ...
      database.getExecutor().execute(() -> {
        collection.deleteMany(Filters.eq("dependency", "zBedWars"));
        ...
        Cosmetic.listCosmetics().forEach(cosmetic -> {
          documents.add(
              new Document("_id", cosmetic.getLootChestsID()).append("name", "&f" + cosmetic.getName() + " &7(Bed Wars - " + cosmetic.getType().getName(cosmetic.getIndex()) + ")")
                  ...
                  .append("action", "bw mb give {player} " + cosmetic.getLootChestsID() + " ; bw mb give {player} " + cosmetic.getLootChestsID()).append("dependency", "zBedWars"));
        });
      ...
      if (Database.getInstance() instanceof MySQLDatabase) {
        ((MySQLDatabase) Database.getInstance()).update("DELETE FROM `kMysteryBoxContent` WHERE `dependency` = ?", "zBedWars");
      } else {
        ((HikariDatabase) Database.getInstance()).update("DELETE FROM `kMysteryBoxContent` WHERE `dependency` = ?", "zBedWars");
      }
      ...
      Cosmetic.listCosmetics().forEach(
          cosmetic -> query.append("('").append(cosmetic.getLootChestsID()).append("', '").append("&f").append(cosmetic.getName()).append(" &7(Bed Wars - ")
              ...
              .append(cosmetic.getLootChestsID()).append(" ; bw mb give {player} ").append(cosmetic.getLootChestsID()).append("', 'zBedWars'),"));
        ...
        if (Database.getInstance() instanceof MongoDBDatabase) {
          ...
          try {
            MongoCursor<Document> rs =
                database.getExecutor().submit(() -> database.getDatabase().getCollection("kMysteryBoxContent").find(new Document("dependency", "zBedWars")).projection(fields(include("_id"))).cursor()).get();
            while (rs.hasNext()) {
          ...
          CachedRowSet rs;
          if (Database.getInstance() instanceof MySQLDatabase) {
            rs = ((MySQLDatabase) Database.getInstance()).query("SELECT * FROM `kMysteryBoxContent` WHERE `dependency` = ?", "zBedWars");
          } else {
            rs = ((HikariDatabase) Database.getInstance()).query("SELECT * FROM `kMysteryBoxContent` WHERE `dependency` = ?", "zBedWars");
          }
