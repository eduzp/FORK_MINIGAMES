...
import mc.twilight.lobby.lobby.ServerEntry;
...
public class NPCPlayCommand extends SubCommand {
  ...
  public void perform(Player player, String[] args) {
    if (args.length == 0) {
      player.sendMessage(" \n§eAjuda\n \n§6/zl npcjogar adicionar [id] [entry] §f- §7Adicionar NPC.\n§6/zl npcjogar remover [id] §f- §7Remover NPC.\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("adicionar")) {
      if (args.length <= 2) {
        player.sendMessage("§cUtilize /zl npcjogar adicionar [id] [entry]");
        return;
      }
      ...
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /zl npcjogar remover [id]");
        return;
      }
