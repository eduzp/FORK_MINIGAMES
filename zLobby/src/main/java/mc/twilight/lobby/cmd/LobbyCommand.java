package mc.twilight.lobby.cmd;

import mc.twilight.lobby.Main;
import mc.twilight.lobby.cmd.kl.*;
...
public class LobbyCommand extends Commands {
  ...
  public void perform(CommandSender sender, String label, String[] args) {
    if (!sender.hasPermission("zlobby.cmd.lobby")) {
      sender.sendMessage("§6zLobby §bv" + Main.getInstance().getDescription().getVersion() + " §7Feito por eduzp.");
      return;
    }
  ...
  private void sendHelp(CommandSender sender, int page) {
    ...
    for (int index = 0; index < commands.size(); index++) {
      ...
      pages.get(currentPage).append("§6/zl ").append(commands.get(index).getUsage()).append(" §f- §7").append(commands.get(index).getDescription()).append("\n");
    }
