...
public class NPCDeliveryCommand extends SubCommand {
  ...
  public void perform(Player player, String[] args) {
    if (args.length == 0) {
      player.sendMessage(" \n§eAjuda\n \n§6/zl npcentregas adicionar [id] §f- §7Adicionar NPC.\n§6/zl npcentregas remover [id] §f- §7Remover NPC.\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("adicionar")) {
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /zl npcentregas adicionar [id]");
        return;
      }
      ...
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /zl npcentregas remover [id]");
        return;
      }
