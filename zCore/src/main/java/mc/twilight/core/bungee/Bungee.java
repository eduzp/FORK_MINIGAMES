package mc.twilight.core.bungee;

import com.google.common.io.ByteArrayDataOutput;
import com.google.common.io.ByteStreams;
import mc.twilight.core.bungee.cmd.Commands;
import mc.twilight.core.bungee.listener.Listeners;
...
public class Bungee extends Plugin {
  ...
  public static void sendRole(ProxiedPlayer player, String sound) {
    ...
    player.getServer().sendData("zCore", out.toByteArray());
  }
  ...
  public void onEnable() {
    ...
    getProxy().registerChannel("zCore");
  ...
  public void saveDefaultConfig() {
    for (String fileName : new String[]{"config", "roles", "utils"}) {
      File file = new File("plugins/zCore/" + fileName + ".yml");
      if (!file.exists()) {
  ...
  private void setupRoles() {
    try {
      if (utils.get("fake.role") instanceof String) {
        ...
        YamlConfiguration.getProvider(YamlConfiguration.class).save(utils, new File("plugins/zCore/utils.yml"));
      }
    } catch (IOException ex) {
      ex.printStackTrace();
    }
