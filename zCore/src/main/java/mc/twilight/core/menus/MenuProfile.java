...
import mc.twilight.core.menus.profile.*;
...
public class MenuProfile extends PlayerMenu {
  ...
  public MenuProfile(Profile profile) {
    ...
    this.setItem(10, BukkitUtils.putProfileOnSkull(this.player, BukkitUtils.deserializeItemStack(
        "SKULL_ITEM:3 : 1 : nome>&aInformações Pessoais : desc>&fRank: " + Role.getRoleByName(profile.getDataContainer("zCoreProfile", "role").getAsString())
            .getName() + "\n&fCash: &b" + StringUtils.formatNumber(profile.getStats("zCoreProfile", "cash")) + "\n \n&fCadastrado: &7" + SDF.format(profile.getDataContainer("zCoreProfile", "created").getAsLong()) + "\n&fÚltimo acesso: &7" + SDF
            .format(profile.getDataContainer("zCoreProfile", "lastlogin").getAsLong()))));
