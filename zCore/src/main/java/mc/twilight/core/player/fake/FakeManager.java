package mc.twilight.core.player.fake;

import com.comphenix.protocol.wrappers.WrappedGameProfile;
import com.comphenix.protocol.wrappers.WrappedSignedProperty;
import mc.twilight.core.Core;
import mc.twilight.core.libraries.profile.Mojang;
...
public class FakeManager {
  ...
  private static final Pattern REAL_PATTERN = Pattern.compile("(?i)zcorefakereal:\\w*"), NOT_CHANGE_PATTERN = Pattern.compile("(?i)zcorenotchange:\\w*");
  ...
  public static String replaceNickedChanges(String original) {
    ...
    for (String name : listNicked()) {
      ...
      while (matcher.find()) {
        replaced = replaced.replaceFirst(Pattern.quote(matcher.group()), Matcher.quoteReplacement("zcorenotchange:" + name));
      }
    }
  ...
  public static String replaceNickedPlayers(String original, boolean toFake) {
    ...
    for (String name : listNicked()) {
      ...
      while (matcher.find()) {
        ...
        replaced = replaced.replaceFirst(Pattern.quote(found), Matcher.quoteReplacement("zcorenotchange:" + (backup.size() - 1)));
      }
    ...
    while (matcher.find()) {
      String found = matcher.group();
      replaced = replaced.replaceFirst(Pattern.quote(matcher.group()), Matcher.quoteReplacement(backup.get(Integer.parseInt(found.replace("zcorenotchange:", "")))));
    }
