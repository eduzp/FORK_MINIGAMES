package mc.twilight.core.database;
...
import mc.twilight.core.database.cache.RoleCache;
import mc.twilight.core.database.conversor.MongoDBConversor;
...
import mc.twilight.core.reflection.acessors.MethodAccessor;
...
public class MongoDBDatabase extends Database {
  ...
  public MongoDBDatabase(String mongoURL) {
    ...
    this.tables =
        DataTable.listTables().stream().map(DataTable::getInfo).map(DataTableInfo::name).filter(name -> !name.equalsIgnoreCase("zCoreProfile")).collect(Collectors.toList());
  ...
  public void setupBoosters() {
    if (!Manager.BUNGEE) {
      MongoCollection<Document> collection = this.database.getCollection("zCoreNetworkBooster");
      for (String mg : Core.minigames) {
        if (collection.find(new BasicDBObject("_id", mg)).first() == null) {
          this.executor.execute(() -> collection.insertOne(new Document("_id", mg).append("booster", "eduzp").append("multiplier", 1.0).append("expires", 0L)));
        }
      }
    }
  }
  ...
  public void setBooster(String minigame, String booster, double multiplier, long expires) {
    this.executor.execute(() -> this.database.getCollection("zCoreNetworkBooster")
  ...
  public NetworkBooster getBooster(String minigame) {
    try {
      Document document = this.executor.submit(() -> this.database.getCollection("zCoreNetworkBooster").find(new BasicDBObject("_id", minigame)).first()).get();
      if (document != null) {
  ...
  public void openConnection() {
    ...
    this.database = this.client.getDatabase("zCore");
  ...
  public Map<String, Map<String, DataContainer>> load(String name) throws ProfileLoadException {
    ...
    if (document != null) {
      ...
      for (String key : document.keySet()) {
        ...
        tableMap.get("zCoreProfile").put(key, new DataContainer(document.get(key)));
      }
    } else {
