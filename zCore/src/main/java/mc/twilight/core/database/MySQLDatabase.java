...
public class MySQLDatabase extends Database {
  ...
  public MySQLDatabase(String host, String port, String dbname, String username, String password, boolean mariadb, boolean skipTables) {
    ...
    if (!skipTables) {
      this.update(
          "CREATE TABLE IF NOT EXISTS `zCoreNetworkBooster` (`id` VARCHAR(32), `booster` TEXT, `multiplier` DOUBLE, `expires` LONG, PRIMARY KEY(`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE utf8_bin;");
  ...
  public void setupBoosters() {
    if (!Manager.BUNGEE) {
      for (String mg : Core.minigames) {
        if (query("SELECT * FROM `zCoreNetworkBooster` WHERE `id` = ?", mg) == null) {
          execute("INSERT INTO `zCoreNetworkBooster` VALUES (?, ?, ?, ?)", mg, "eduzp", 1.0, 0L);
        }
      }
    }
  }
  ...
  public void setBooster(String minigame, String booster, double multiplier, long expires) {
    execute("UPDATE `zCoreNetworkBooster` SET `booster` = ?, `multiplier` = ?, `expires` = ? WHERE `id` = ?", booster, multiplier, expires, minigame);
  }
  
  @Override
  public NetworkBooster getBooster(String minigame) {
    try (CachedRowSet rs = query("SELECT * FROM `zCoreNetworkBooster` WHERE `id` = ?", minigame)) {
      if (rs != null) {
  ...
  public String getRankAndName(String player) {
    try (CachedRowSet rs = query("SELECT `name`, `role` FROM `zCoreProfile` WHERE LOWER(`name`) = ?", player.toLowerCase())) {
      if (rs != null) {
  ...
  public boolean getPreference(String player, String id, boolean def) {
    ...
    try (CachedRowSet rs = query("SELECT `preferences` FROM `zCoreProfile` WHERE LOWER(`name`) = ?", player.toLowerCase())) {
      if (rs != null) {
  ...
  public String exists(String name) {
    try {
      return this.query("SELECT `name` FROM `zCoreProfile` WHERE LOWER(`name`) = ?", name.toLowerCase()).getString("name");
    } catch (Exception ex) {
      return null;
    }
  }
