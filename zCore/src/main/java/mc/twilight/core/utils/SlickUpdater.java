...
public class SlickUpdater {
  ...
  public void downloadUpdate(Player player) {
    ...
    try {
      File file = new File("plugins/zCore/update", "zCore.jar");
      if (!file.getParentFile().exists()) {
        file.getParentFile().mkdirs();
      }
      HttpsURLConnection connection = (HttpsURLConnection) new URL("https://www.slickcollections.com.br/download/zCore.jar").openConnection();
      connection.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.11 (KHTML, like Gecko) Chrome/23.0.1271.95 Safari/537.11");
