...
import static mc.twilight.core.player.fake.FakeManager.ALEX;
import static mc.twilight.core.player.fake.FakeManager.STEVE;
...
public class FakeCommand extends Commands {
  ...
  public void perform(CommandSender sender, String label, String[] args) {
    ...
    if (!player.hasPermission("zcore.cmd.fake") || (label.equalsIgnoreCase("fakel") && !player.hasPermission("zcore.cmd.fakelist"))) {
    ...
    if (label.equalsIgnoreCase("fake")) {
      ...
      for (int index = 0; index < nicked.size(); index++) {
        sb.append("§c").append(nicked.get(index)).append(" §fé na verdade ").append("§azcorefakereal:").append(nicked.get(index)).append(index + 1 == nicked.size() ? "" : "\n");
      }
